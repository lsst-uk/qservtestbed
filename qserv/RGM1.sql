...
and ( l.decl >= minDec and l.decl <= maxDec) AND (priOrSec<=0 OR priOrSec=frameSetID) AND yClass = -1.0 AND yppErrBits = 0 
AND j_1Class = -1.0 AND j_1ppErrBits = 0 AND hClass = -1.0 AND hppErrBits = 0 AND (j_2Class=-1.0 OR j_2Class = -9999) 
AND j_2ppErrBits <= 0 AND (kClass=-1.0 OR kClass = -9999)  AND kppErrBits <= 0 AND yXi BETWEEN -1.0 AND 1.0 AND yEta BETWEEN -1.0 
